...
public class Day15 {
    ...
    private static void executeProgram(List<String> numbers) throws InterruptedException {
        Thread thread = new Thread(new Day5.ExecuteProgramTask(new ArrayList<>(numbers), inputQueue, outputQueue));
        thread.start();
    ...
    private static Optional<MovementResponse> move(MovementCommand movementCommand) {
        ...
        inputQueue.add(movementCommand.toInputValue());
        Optional<Long> outputValue = getNextOutputValue() ;
